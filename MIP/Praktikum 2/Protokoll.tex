...
Die Ebenen-Daten liegen in Form von Arrays vor und zu jedem Zeitpunkt $t$ bestimmt der Agent alle drei \todo{Oben ist von 4 Ebenen die rede ?} Ebenen und hält dieses Tupel in einer geeigneten Datenstruktur fest. Die Ebenen-Daten werden genutzt um zur Laufzeit das Empfinden des Agenten zu berechnen.
...
\item Definition der Parameter zu Gewichtung einzelner Ebenen-Aspekte für den Agenten \todo{werden hier die Parameter aus 2 geändert ? Wenn ja wieso ?}
\item Agent läuft vordefinierte Wege
\item Berechnung der Ebenen-Daten für jeden Zeitpunk $t$
\item Sicherung und Auswertung der Messergebnisse
